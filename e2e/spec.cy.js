describe('load Calculator app', () => {
  ...
  // seems more efficient then the one above
  // it('all buttons are clickable', () => {
  ...
  it('all number buttons will update display with the correct value', () => {
    ...
    cy.get('button[data-cy|="num"]').each(($btn) => {
      ...
      cy.get($btn).click().invoke('text').then(($digit) => {
        ...
        // other solution to the above is to leave the digits on the display
  ...
  // this would be a more manula solution to the above
  // where I need to copy paste and update the digits 10x
